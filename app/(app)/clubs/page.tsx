...
export default function ClubsEventsPage() {
  ...
  const CreatePostForm = () => (
    <div className="theme-card overflow-hidden">
      <button onClick={() => setShowCreatePost(p => !p)}
        className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-[rgb(var(--muted)/0.5)] transition-colors">
        ...
        <span className="text-sm text-[rgb(var(--muted-fg))] flex-1 text-left">Share an announcement, event or update…</span>
        <Plus className={cn("w-4 h-4 text-[rgb(var(--muted-fg))] transition-transform", showCreatePost && "rotate-45")} />
      </button>
          ...
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }}
            ...
            <div className="px-4 pb-4 space-y-3 border-t border-[rgb(var(--border))]">
              {adminSocieties.length > 1 && (
                <div className="pt-3">
                  <label className="text-xs font-medium text-[rgb(var(--muted-fg))] mb-1.5 block">Posting as</label>
                  ...
                  {POST_TYPES.map(pt => (
                    <button key={pt.value} onClick={() => setPostType(pt.value)}
                      className={cn("flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-colors border flex-1 justify-center",
                        postType === pt.value ? "bg-[rgb(var(--primary)/0.12)] border-[rgb(var(--primary)/0.3)] text-[rgb(var(--primary))]"
              ...
              {postType === "event" && (
                <div>
                  <label className="text-xs font-medium text-[rgb(var(--muted-fg))] mb-1.5 block">Event date & time</label>
              ...
              <div className="flex items-center gap-2 pt-1">
                <button onClick={() => fileRef.current?.click()} disabled={postImages.length >= 30}
                  ...
                  <ImageIcon className="w-3.5 h-3.5" /> Photos {postImages.length > 0 && `(${postImages.length}/30)`}
                </button>
                <div className="flex-1" />
                {postError && <p className="text-xs text-red-400">{postError}</p>}
                <button onClick={() => setShowCreatePost(false)}
                  className="px-3 py-2 rounded-xl text-xs text-[rgb(var(--muted-fg))] hover:bg-[rgb(var(--muted))] transition-colors">Cancel</button>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-5xl mx-auto pb-16 space-y-6">
      ...
      <div className="flex gap-1 p-1 rounded-2xl bg-[rgb(var(--muted))]">
        {([
          ["societies", "Societies",   Building2],
          ["events",    "Events Feed", Calendar],
        ...
        <div className="space-y-5">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-none">
              {CATEGORIES.map(c => (
                <button key={c} onClick={() => setCategory(c)}
                  className={cn(
                    "px-4 h-9 rounded-xl text-sm font-medium capitalize flex-shrink-0 transition-all border",
                    category === c
            ...
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                <input value={societySearch} onChange={e => setSocietySearch(e.target.value)}
                  ...
                  className="w-full h-10 pl-10 pr-4 rounded-xl text-sm bg-[rgb(var(--input))] border border-[rgb(var(--border))] outline-none focus:ring-2 focus:ring-[rgb(var(--ring))] text-[rgb(var(--fg))] placeholder:text-[rgb(var(--muted-fg))]" />
              </div>

              {/* University combobox */}
              <div ref={uniRef} className="relative w-52 flex-shrink-0">
                <button onClick={() => setUniDropdownOpen(p => !p)}
                  ...
                  <span className={cn("truncate flex-1 text-sm", selectedUni !== "all" ? "text-[rgb(var(--primary))]" : "text-[rgb(var(--muted-fg))]")}>
                    {selectedUniLabel}
                  </span>
          ...
          {societiesLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[1,2,3,4,5,6].map(i => (
                <div key={i} className="h-52 rounded-2xl bg-[rgb(var(--muted))] animate-pulse" />
              ))}
            </div>
            ...
            <div className="theme-card p-16 text-center">
              ...
              <Link href="/societies/register"
                className="inline-flex items-center gap-2 mt-4 px-4 py-2 rounded-xl bg-[rgb(var(--primary)/0.1)] text-[rgb(var(--primary))] text-sm font-semibold hover:bg-[rgb(var(--primary)/0.18)] transition-colors">
                <Plus className="w-4 h-4" /> Register Society
              </Link>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSocieties.map((s, i) => (
                <motion.div key={s.id}
                  ...
                  className="theme-card overflow-hidden flex flex-col hover:border-[rgb(var(--primary)/0.35)] hover:shadow-lg transition-all duration-200 group">

                  {/* Top accent bar */}
                  <div className="h-1 w-full bg-gradient-to-r from-[rgb(var(--primary))] to-[rgb(var(--accent))]" />

                  <div className="p-5 flex flex-col gap-3 flex-1">
                    {/* Logo + category */}
                    <div className="flex items-start justify-between gap-2">
                      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[rgb(var(--primary))] to-[rgb(var(--accent))] flex items-center justify-center text-white text-2xl font-bold overflow-hidden flex-shrink-0 shadow-md">
                        {s.logo_url
                          ? <img src={s.logo_url} alt="" className="w-full h-full object-cover" />
                          : s.name.charAt(0)}
                      </div>
                      <span className={cn("text-[10px] font-semibold px-2.5 py-1 rounded-full capitalize", CAT_COLORS[s.category] ?? CAT_COLORS.general)}>
                        {s.category}
                      </span>
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/societies/${s.id}`}>
                        <h3 className="font-bold text-sm leading-snug group-hover:text-[rgb(var(--primary))] transition-colors line-clamp-1">
                          {s.name}
                        </h3>
                      </Link>
                      {s.university && (
                        <p className="text-[11px] text-[rgb(var(--muted-fg))] flex items-center gap-1 mt-0.5">
                          <Building2 className="w-3 h-3 flex-shrink-0" /> {s.university.short_name}
                        </p>
                      )}
                      {s.description && (
                        <p className="text-xs text-[rgb(var(--muted-fg))] mt-2 line-clamp-2 leading-relaxed">
                          {s.description}
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-[rgb(var(--border))]">
                      <span className="flex items-center gap-1 text-xs text-[rgb(var(--muted-fg))]">
                        <Users className="w-3.5 h-3.5" /> {s.member_count.toLocaleString()}
                      </span>
                      <div className="flex items-center gap-2">
                        {joinedIds.has(s.id) ? (
                          <button onClick={() => leave(s.id)} disabled={joining === s.id}
                            className="text-xs px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 font-medium hover:bg-emerald-500/20 transition-colors">
                            {joining === s.id ? "…" : "✓ Joined"}
                          </button>
                        ) : (
                          <button onClick={() => join(s.id)} disabled={joining === s.id}
                            className="text-xs px-3 py-1.5 rounded-lg bg-[rgb(var(--primary)/0.1)] text-[rgb(var(--primary))] font-medium hover:bg-[rgb(var(--primary)/0.2)] transition-colors">
                            {joining === s.id ? "…" : "Join"}
                          </button>
                        )}
                        <Link href={`/societies/${s.id}`}
                          className="text-xs px-3 py-1.5 rounded-lg border border-[rgb(var(--border))] text-[rgb(var(--muted-fg))] hover:text-[rgb(var(--fg))] hover:border-[rgb(var(--primary)/0.3)] transition-colors">
                          View
                        </Link>
                      </div>
                    </div>
      ...
      {tab === "events" && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6 items-start">
          ...
          <div className="space-y-4 min-w-0">
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <button onClick={() => setShowUpcomingOnly(p => !p)}
                className={cn(
                  "flex items-center gap-2 px-4 h-10 rounded-xl text-sm font-medium transition-all border flex-shrink-0",
                  showUpcomingOnly
                    ? "bg-[rgb(var(--primary))] text-[rgb(var(--primary-fg))] border-transparent"
            ...
            {events.length === 0 && !eventsLoading ? (
              <div className="theme-card p-16 text-center">
                ...
                <p className="text-sm text-[rgb(var(--muted-fg))] mt-1">
                  {joinedIds.size === 0
                    ? "Join some societies to see their updates here!"
                    : "The societies you've joined haven't posted yet."}
                </p>
                <button onClick={() => setTab("societies")}
                  className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[rgb(var(--primary)/0.1)] text-[rgb(var(--primary))] text-sm font-semibold hover:bg-[rgb(var(--primary)/0.18)] transition-colors">
                  <Users className="w-4 h-4" /> Browse Societies
                </button>
              </div>
            ) : (
              <div className="space-y-4">
                {filteredEvents.map((ev, i) => {
                  ...
                  return (
                    <motion.article key={ev.id}
                      ...
                      transition={{ delay: Math.min(i * 0.04, 0.3) }}
                      className={cn("theme-card overflow-hidden", upcoming && "ring-1 ring-[rgb(var(--primary)/0.25)]")}>

                      {/* Header */}
                      <div className="flex items-center gap-3 p-4">
                        <Link href={`/societies/${ev.society?.id}`} className="flex-shrink-0">
                          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[rgb(var(--primary))] to-[rgb(var(--accent))] flex items-center justify-center text-white font-bold text-sm overflow-hidden">
                            {ev.society?.logo_url
                              ? <img src={ev.society.logo_url} alt="" className="w-full h-full object-cover" />
                              : ev.society?.name.charAt(0) ?? "?"}
                          </div>
                        </Link>
                        <div className="flex-1 min-w-0">
                          <Link href={`/societies/${ev.society?.id}`}
                            className="font-bold text-sm hover:text-[rgb(var(--primary))] transition-colors line-clamp-1">
                            {ev.society?.name}
                          </Link>
                          <p className="text-[11px] text-[rgb(var(--muted-fg))]">
                            {ev.author?.full_name} · {new Date(ev.created_at).toLocaleDateString("en-PK", { day: "numeric", month: "short" })}
                          </p>
                        </div>
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                          ...
                          <span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full capitalize border", typeStyle)}>
                            {typeLabel}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="px-4 pb-3 space-y-1.5">
                        ...
                        {ev.event_date && (
                          <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-[rgb(var(--primary))] bg-[rgb(var(--primary)/0.08)] px-3 py-1.5 rounded-lg mt-1">
                      ...
                      {/* Images */}
                      {imgs.length > 0 && <ImageGrid urls={imgs} />}
                      ...
                      <div className="flex items-center justify-between px-4 py-3 border-t border-[rgb(var(--border))]">
                        <span className="flex items-center gap-1.5 text-xs text-[rgb(var(--muted-fg))]">
                          <Star className="w-3.5 h-3.5" /> {ev.likes}
                        </span>
                        <div className="flex items-center gap-3">
                          ...
                          <Link href={`/societies/${ev.society?.id}`}
                            className="flex items-center gap-1 text-xs font-semibold text-[rgb(var(--primary))] hover:opacity-80 transition-opacity">
                ...
                <div id="events-end-trigger" className="h-10 flex items-center justify-center">
                  ...
                  {!hasMoreEvents && events.length > 0 && (
                    <p className="text-xs text-[rgb(var(--muted-fg))]">You&apos;ve seen all posts.</p>
                  )}
                </div>
              </div>
            )}
          ...
          <div className="space-y-4 lg:sticky lg:top-4">
            {/* Create post — society admins */}
            {adminSocieties.length > 0 && <CreatePostForm />}

            {/* Quick tip when no societies joined */}
            {joinedIds.size === 0 && (
              <div className="theme-card p-5 space-y-3">
                <p className="font-semibold text-sm flex items-center gap-2">
                  <Users className="w-4 h-4 text-[rgb(var(--primary))]" /> Get started
                </p>
                <p className="text-xs text-[rgb(var(--muted-fg))] leading-relaxed">
                  Join societies to see their announcements and events appear here in your feed.
                </p>
                <button onClick={() => setTab("societies")}
                  className="w-full py-2 rounded-xl text-xs font-semibold bg-[rgb(var(--primary)/0.1)] text-[rgb(var(--primary))] hover:bg-[rgb(var(--primary)/0.18)] transition-colors">
                  Browse Societies →
                </button>
              </div>
            )}
            ...
            {events.filter(e => isUpcoming(e.event_date)).length > 0 && (
              <div className="theme-card p-4 space-y-3">
                <p className="font-semibold text-sm flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-[rgb(var(--primary))]" /> Upcoming Events
                </p>
                  ...
                  {events.filter(e => isUpcoming(e.event_date)).slice(0, 4).map(ev => (
                    <div key={ev.id} className="flex items-start gap-2.5 p-2 rounded-xl bg-[rgb(var(--muted)/0.5)] hover:bg-[rgb(var(--muted))] transition-colors">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-[rgb(var(--muted-fg))]">{formatEventDate(ev.event_date)}</p>
                      </div>
                    </div>
                  ))}
                </div>
