...
export default function RegisterSocietyPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: newSociety, error: dbErr } = await supabase.from("societies").insert({
      name: form.name.trim(),
    ...
    // Automatically add the creator as the first member (role: creator)
    if (newSociety) {
      ...
      <div className="max-w-md mx-auto text-center py-20">
        ...
        >
          ...
          <div className="flex gap-3 justify-center">
            <Link href="/clubs" className="px-6 py-2.5 rounded-xl bg-[rgb(var(--primary))] text-white font-semibold text-sm hover:opacity-90 transition-opacity">
              Browse Societies
            </Link>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl mx-auto pb-16">
      {/* Back */}
      <Link href="/clubs" className="inline-flex items-center gap-2 text-sm text-[rgb(var(--muted-fg))] hover:text-[rgb(var(--fg))] transition-colors mb-6">
        <ArrowLeft className="w-4 h-4" /> Back to Clubs & Events
      </Link>
